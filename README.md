...
- **Output:** The network outputs an 88-element vector, reshaped to 8 sequence slots. Each slot is an 11-way classification: Class 0 (Padding), Class 1 (A), Class 2 (B), and Classes 3-10 (Run counts 1-8).
...
## 6. Limitations and Scope Acceptance

The model's constraints are intentional and serve the primary objective of demonstrating observational equivalence.

Because recurrent networks rely on continuous probabilistic calculations—which inevitably suffer from statistical drift over long sequences—this project bypasses those vulnerabilities using a feedforward network on a bounded universe. The model functions practically as a truth-table lookup system.
